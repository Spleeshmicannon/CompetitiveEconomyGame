...
import Components.DataStructures.Enums.*;
...
public class MainGame {
    ...
    public static void CreateGui() {
        ...
        initTable2(Tables.returnTable(Tables.Education), educationPanel);
        initTable2(Tables.returnTable(Tables.Economy), economyPanel);
        initTable2(Tables.returnTable(Tables.Science), sciencePanel);
        initTable2(Tables.returnTable(Tables.Military), militaryPanel);
        initTable2(Tables.returnTable(Tables.Production), productionPanel);
        ...
        JButton[] btns = new JButton[] { // creating various buttons
                ...
                Gui.CreateButton("Ad Campaign", WIDTH / 14 + 100, 320, 120, 35, e -> {
                    playEcon.AdCampaign();
                    labels[labels.length-1].setText(String.format("$%d", playEcon.AdCost()));
                    RefreshGui();
                })
        };
    ...
    public static void initLabels() {
        labels = new JLabel[]{ // creating various text elements
                ...
                Gui.CreateText(String.format(" $%d", playEcon.AdCost()), WIDTH / 14 + 220, 320, 120, 35, 20)
        };
    }

    public static void initTable2(JTable table, TabPanel panel) {

        table.setRowHeight(25);
        setTableColumnWidths(table, 250, 75, 75, 75, 75);
        table.setBounds(0, 0, 550, 100);
        panel.addComponents(table);
    }
    ...
    public static void initTabMenu() {
        ...
        JTabbedPane secondaryTabPane = new JTabbedPane();
        secondaryTabPane.setBounds(-4,-3,554,493);
        //secondaryTabPane.setBorder(false);
        ...
        for (Map.Entry<TabPanel, String> panel : panelList.entrySet()) {
            panel.getKey().addComponents(
                    Gui.CreateText(panel.getValue(), 10, -50, 100, 200, 20));
        }
        ...
        localPoliticsPanel.add(governmentChart);
        politicsPanel.add(secondaryTabPane);
        win.getContentPane().add(politicsPanel);

        secondaryTabPane.addTab("International", intPoliticsPanel);
        secondaryTabPane.addTab("Local", localPoliticsPanel);
